...
class SoChiHuyGiaoDien:
    ...
    def render_tructhe(df):
        ...
        with col_nav:
            ...
            # 🟢 TÍNH NĂNG MỞ CHO CON CHÁU TỰ TẢI ẢNH (ĐIỀU KHIỂN BỞI CÔNG TẮC QUẢN TRỊ)
            if st.session_state.get("cho_phieu_tai_anh", False):
    ...
    def render_quantri(df):
        ...
        # Cập nhật mật khẩu quản trị thành admin12345
        if mat_khau == "admin12345":
            ...
            # Công tắc bật/tắt toàn cục để mở/khóa tính năng tự tải ảnh chân dung
            cho_phep = st.toggle(
                "Mở chế độ cho phép mọi người tự tải ảnh chân dung", 
                value=st.session_state.get("cho_phieu_tai_anh", False),
                key="toggle_cho_phep_tai_anh"
            )
            st.session_state["cho_phieu_tai_anh"] = cho_phep
            
            if cho_phep:
                st.warning("⚠️ Đang MỞ: Bất kỳ ai vào mục 'Trực Hệ' đều có thể chọn tên mình và tự tải ảnh chân dung lên hệ thống.")
            else:
                st.info("🔒 Đang KHÓA: Tính năng tự tải ảnh đang tạm vô hiệu hóa để bảo mật cơ sở dữ liệu.")
